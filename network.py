...
resnet_rn_dict = {"ResNet18": resnet_rn.resnet18, "ResNet34": resnet_rn.resnet34, "ResNet50": resnet_rn.resnet50,
               "ResNet101": resnet_rn.resnet101, "ResNet152": resnet_rn.resnet152}
...
class ResNetFc(nn.Module):
  def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
    ...
    model_resnet = resnet_rn_dict[resnet_name](pretrained=True)
    ...
    if new_cls:
        if self.use_bottleneck:
            if args.dset == 'visda' and args.net == 'ResNet50':
                self.bottleneck = nn.Sequential(nn.Linear(model_resnet.fc.in_features, bottleneck_dim))
            else:
                self.bottleneck = nn.Sequential(nn.Linear(model_resnet.fc.in_features, bottleneck_dim),
                                 RN1d(bottleneck_dim),
                                 nn.ReLU(inplace=True))
    ...
    if self.new_cls:
        if self.use_bottleneck:
            if args.norm_type == 'rn':
                my_weight = [v for k, v in self.feature_layers.named_parameters() if 'my' in k]
                features = [v for k, v in self.feature_layers.named_parameters() if not 'my' in k ]
                parameter_list = [{"params": features, "lr_mult": 1, 'decay_mult': 2}, \
                                  {"params": my_weight, "lr_mult": args.lr_mult, 'decay_mult': 2}, \
                                  {"params": self.bottleneck.parameters(), "lr_mult": 10, 'decay_mult': 2}, \
                                  {"params": self.fc.parameters(), "lr_mult": 10, 'decay_mult': 2}]
...
class Domain_Classifier(nn.Module):
    def __init__(self, in_feature):
        ...
        self.domain_classifer = nn.Linear(in_feature, 2)

    def forward(self, x):
        out = self.domain_classifer(x)
        return  out
